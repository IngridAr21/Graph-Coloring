...
public class GameMode2Controller {
    ...
    public void graphTouched() {
        ...
        if (calculator.getExactChromaticNumber() < playerChromaticNumber) {
            messageLabel.setText("You didn't reach the minimal chromatic number yet. Try more");
            return;
        }
    ...
    public void hint() {
        ...
        for (int i = 0; i < mask.length; i++) { // picks first vertex with not the same color as int best color distribution
    ...
    @FXML
    public void timer() {
        timerCounter = graph.getEdgesNumber() * 5 + 30; // timer value in seconds
    ...
    private double calculateScore() {
        ...
        if (isGraphColored()) {
            return Math.max(50 * graph.getVerticesNumber() + 10 * (initialTime - timerCounter) / graph.getVerticesNumber() - (100 * hintCounter), 0);
        }
        else {
            return Math.max(100 / (graph.getChromaticNumber() - getPlayerChromaticNumber() - 100 * hintCounter), 0);
        }
    }
